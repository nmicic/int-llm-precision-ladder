...
/* Experimental inference-only storage mode used by the precision
 * ladder.  Every model weight is a signed int16 F12 code; activations and all
 * arithmetic results remain Q16.48 fixed_t.  The normal training and MGW
 * paths leave this flag zero and are unchanged. */
#define MGPT_PACKED_FRACTION_BITS 12
#ifdef MGPT_PACKED_F12
static int mgpt_weights_packed_f12 = 0;

static inline fixed_t mgpt_weight_at(const fixed_t *weights, size_t index) {
    if (!mgpt_weights_packed_f12)
        return weights[index];
    const int16_t *codes = (const int16_t *)(const void *)weights;
    return (fixed_t)((int64_t)codes[index] *
                     (INT64_C(1) << (FP_PRECISION -
                                     MGPT_PACKED_FRACTION_BITS)));
}
#else
static inline fixed_t mgpt_weight_at(const fixed_t *weights, size_t index) {
    return weights[index];
...
#ifdef MGPT_PACKED_F12
    if (mgpt_weights_packed_f12) {
        const int16_t *codes = (const int16_t *)(const void *)w;
        for (int r = 0; r < nout; r++) {
            fixed_t s = 0;
            const int16_t *row = codes + (size_t)r * (size_t)nin;
            for (int c = 0; c < nin; c++) {
                fp_w128 product = fp_w_muls(x[c], (int64_t)row[c]);
                s += (fixed_t)fp_w_to_i64(fp_w_asr(
                    product, MGPT_PACKED_FRACTION_BITS));
            }
            out[r] = s;
        }
        return;
...
#ifdef MGPT_MATVEC_PROBE
/*
 * Isolated fixture hook for the MCU matvec experiment.  Normal builds do
 * not contain a call or symbol.  The probe receives the real post-RMSNorm
 * activation and rounded-wide MLP tensor immediately before linear_fwd.
 */
void mgpt_matvec_probe(const fixed_t *activation, const fixed_t *weights,
                       int nout, int nin);
#endif

#ifdef MGPT_PRECISION_TRACE
/*
 * Stable little-endian FNV-1a over raw fixed-point words.  This trace is
 * compiled only in the isolated precision-ladder spike; it does not alter
 * model arithmetic or PRNG state.
 */
static uint64_t precision_trace_hash(const fixed_t *values, int count) {
    uint64_t hash = UINT64_C(14695981039346656037);
    for (int i = 0; i < count; i++) {
        uint64_t word = (uint64_t)values[i];
...
#ifdef MGPT_OBSERVE_LOGITS
/* Canonical little-endian hash of every raw pre-temperature logit word.
 * This is a correctness observer only: it neither changes arithmetic nor
 * consumes PRNG state. */
static uint64_t mgpt_observed_logits_hash =
    UINT64_C(14695981039346656037);
static uint32_t mgpt_observed_steps = 0;

void mgpt_observer_reset(void) {
    mgpt_observed_logits_hash = UINT64_C(14695981039346656037);
    mgpt_observed_steps = 0;
}

uint64_t mgpt_observer_hash(void) {
    return mgpt_observed_logits_hash;
}

uint32_t mgpt_observer_steps_count(void) {
    return mgpt_observed_steps;
}

static void mgpt_observe_logits(const fixed_t *values, int count) {
    for (int i = 0; i < count; i++) {
        uint64_t word = (uint64_t)values[i];
        for (int byte = 0; byte < 8; byte++) {
            mgpt_observed_logits_hash ^= word & UINT64_C(0xff);
            mgpt_observed_logits_hash *= UINT64_C(1099511628211);
            word >>= 8;
        }
    }
    mgpt_observed_steps++;
...
static void inference_forward(int token_id, int pos, fixed_t *logits_out) {
    ...
    for (int i = 0; i < N_EMBD; i++)
        x[i] = mgpt_weight_at(wte, (size_t)token_id * N_EMBD + i)
             + mgpt_weight_at(wpe, (size_t)pos * N_EMBD + i);
...
#ifdef MGPT_MATVEC_PROBE
        if (li == 0 && pos == 0)
            mgpt_matvec_probe(xn_m, mlp_fc1[li], MLP_DIM, N_EMBD);
#endif
...
int mgpt_generate_sample(char out[BLOCK_SIZE + 1]) {
    ...
    int this_sample = trace_sample++;
#endif
    fixed_t temperature = FP_ONE / 2; /* 0.5 */
    int slen = 0, token_id = BOS;
    for (int pos = 0; pos < BLOCK_SIZE; pos++) {
#ifdef MGPT_PRECISION_TRACE
        int input_token = token_id;
#endif
        inference_forward(token_id, pos, logits);
#ifdef MGPT_OBSERVE_LOGITS
        mgpt_observe_logits(logits, vocab_size);
#endif
#ifdef MGPT_PRECISION_TRACE
        uint64_t logits_hash = precision_trace_hash(logits, vocab_size);
        int top1 = 0, top2 = vocab_size > 1 ? 1 : 0;
        if (logits[top2] > logits[top1]) {
            int swap = top1;
            top1 = top2;
            top2 = swap;
        }
        ...
        fixed_t top1_value = logits[top1];
        fixed_t top1_margin = top1_value - logits[top2];
#endif
...
#ifdef MGPT_PRECISION_TRACE
        uint64_t probs_hash = precision_trace_hash(probs, vocab_size);
        printf("TRACE s=%d p=%d in=%d out=%d lh=%016llx ph=%016llx "
               "top1=%d topv=%lld margin=%lld\n",
               this_sample, pos, input_token, token_id,
               (unsigned long long)logits_hash,
               (unsigned long long)probs_hash,
               top1, (long long)top1_value, (long long)top1_margin);
#endif
...
#define MGWI_MAGIC      "MGWI"         /* packed signed-int16 weights */
...
int mgpt_load_mem(const void *buf, size_t len) {
    ...
    int packed_f12 = 0;
#ifdef MGPT_PACKED_F12
    packed_f12 = memcmp(hdr.magic, MGWI_MAGIC, MGW_MAGIC_SIZE) == 0;
#endif
    if ((!packed_f12 &&
         memcmp(hdr.magic, MGW_MAGIC, MGW_MAGIC_SIZE) != 0) ||
        hdr.version != (packed_f12 ? MGWI_VERSION : MGW_VERSION) ||
    ...
    for (uint32_t i = 0; i < hdr.num_tensors; i++) {
        ...
        memcpy(&e, p + hdr.index_offset + (uint64_t)i * MGW_INDEX_ENTRY_SIZE,
               sizeof(e));
        e.name[sizeof(e.name) - 1] = 0;
        int is_raw = !packed_f12 || e.reserved == MGWI_RAW_INT64_TAG;
        uint64_t element_bytes = is_raw ? sizeof(int64_t) : sizeof(int16_t);
        if (e.num_elements > UINT64_MAX / element_bytes ||
            e.data_offset % element_bytes != 0 ||
            e.data_offset > (uint64_t)len ||
            e.num_elements * element_bytes > (uint64_t)len - e.data_offset)
            return -1;
        const void *payload = (const void *)(p + e.data_offset);
        const int64_t *data = (const int64_t *)payload;

        if (strcmp(e.name, "tokenizer.uchars") == 0) {
            if (!is_raw || e.num_elements != (uint64_t)(vocab_size - 1))
                return -1;
            for (int j = 0; j < vocab_size - 1; j++)
                uchars_arr[j] = (char)(unsigned char)data[j];
            num_uchars = vocab_size - 1;
        } else if (strcmp(e.name, "rng.state") == 0) {
            if (!is_raw || e.num_elements != 1) return -1;
            ...
            for (int t = 0; t < nw; t++) {
                ...
                if (e.num_elements != (uint64_t)ne ||
                    (packed_f12
                     ? (is_raw || e.reserved != MGPT_PACKED_FRACTION_BITS)
                     : (!is_raw || e.reserved != 0))) return -1;
                *tab[t].slot = (fixed_t *)(uintptr_t)payload;
                num_params += (int)ne;
                found++;
                break;
            }
...
#ifdef MGPT_PACKED_F12
    mgpt_weights_packed_f12 = packed_f12;
#endif
    ATTN_SCALE = fp_inv_sqrt(fp_from_int(HEAD_DIM));
    return 0;
}
